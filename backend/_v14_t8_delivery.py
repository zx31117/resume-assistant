...
_NAME_ALWAYS_EXCLUDE = {".git", ".t8-manifest.json", "__t8_acceptance_worktree"}
...
def copy_clean(source_root: Path, dest_root: Path, manifest: dict):
    ...
    # 注意 rglob 的递归起点仍是 source_root；如果 dest 在里面，
    # 就必须能通过 should_skip 匹配到 dest 的 basename，所以在
    # _NAME_ALWAYS_EXCLUDE 里添加了 "__t8_acceptance_worktree"。
    for src in source_root.rglob("*"):
        if src.is_dir():
            continue
        rel = src.relative_to(source_root)
        parts = rel.parts
        # 根路径任何一级都不在排除目录里（用 rel.parts 逐段检查）
...
def main():
    ...
    #   不再在 docs/versions/v1.4/ 下留副本，避免两个真源互相冲突。
    ...
    print(f"  （docs/versions/v1.4/T8_manifest.json 不再生成，避免双真源冲突）")
