"""V1.4 C2/C3 修正专用脚本：
1. C2：修正文档中对 docs/versions/v1.4/T8_manifest.json 的陈旧引用（统一指向 <delivery-root>/.t8-manifest.json）
2. C3：把 V1.0–V1.4 文档（.md / .json）中真实本机路径替换为通用占位符，保留语义。
...
def _replace_c2_references(text: str) -> str:
    """C2：删除/替换对陈旧 docs/versions/v1.4/T8_manifest.json 的引用。"""
    text = re.sub(
        r"\[(?:docs/versions/v1\.4/)?T8_manifest\.json\]\([^)]*T8_manifest\.json\)",
    ...
    text = re.sub(
        r"`?docs/versions/v1\.4/T8_manifest\.json`?",
