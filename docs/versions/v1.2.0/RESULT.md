# AI Career Resume Assistant V1.2.0 测试验收报告

> 文档角色：V1.2.0 历史实施结果与验收证据  
> 状态：模板路径 A 已验收；JD 驱动路径 B 未完全接入  
> 对应计划：[PLAN.md](./PLAN.md)  
> 阅读提示：本文的“V1.2.0 验收通过”仅覆盖当时定义的路径 A，不等于当前核心 JD → DOCX 已闭环
...
| `to_standard()` 字段迁移 | V1.1.0 旧字段 → V1.2.0 标准字段 |
...
V1.2.0 在 V1.1.0（数据层 / RAG / AI 生成）之上新增「标准化模板渲染层」，把 AI 生成结果稳定输出为可编辑 DOCX。
...
│    末尾统一 to_standard() 迁移 V1.1.0 旧字段 → V1.2.0 标准字段             │
│             │                                                         │
│             ▼                                                         │
...
│    apply_keyword_bold: 保留但不调用（V1.2.0 PDF 复刻版取消关键词加粗）          │
│             │                                                         │
│             ▼                                                         │
...
| API | `api/routes/template.py` | HTTP 接口（list / generate-docx / generate-report） | - | 返回二进制（V1.2.0 返回 path + report） |
...
Step 1  构造 ResumeDocument
  直传 JSON（或 V1.1.0 ResumeContent → to_standard() 迁移旧字段）
...
Step 2  TemplateRenderer.render(resume_doc)
  ...
  └─ 不调用 apply_keyword_bold（V1.2.0 PDF 复刻版取消关键词加粗）
...
| 3 | V1.1.0 JD 驱动路径（Path B）未完全接入 | P1 | ⚠️ |
