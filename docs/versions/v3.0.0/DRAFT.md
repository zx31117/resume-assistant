...
- V3.0.0 面向用户提供受支持的 LLM / Embedding 供应商配置，兼容云端和本地模型；
...
| 生成 bullets | 模型返回后由浏览器校验 | 否 | 每条必须携带 fact_ids |
...
浏览器收到响应后必须再次核对所有 fact_ids 和来源映射。未知 ID、无来源 bullet 或越界结果丢弃并告警。客户端本地 SQL 是 V3.0.0 的最终事实校验点。
...
用户可以在设置中选择受支持的云端供应商、本地模型或自定义兼容 Endpoint，填写配置后通过能力检测即可使用。业务链路不能直接依赖具体供应商 SDK、请求格式或响应结构。
