# AI Career Resume Assistant V3.0.0 草稿：服务器化 Career Memory 与真实市场验证

> 文档角色：远期产品与架构草稿，供用户审核
> 状态：DRAFT，非开发指令，不改变当前版本状态
> 草稿日期：2026-08-19
> 假定前置：V1.5.0 完成事实级内容决策与本地索引收束，V2 完成相关性、生成和排版体验优化
> 核心方向：网页/PWA 提供产品界面；直接身份尽量留在用户设备，语义降敏后的 Career Memory 默认允许账号级服务端持久化；服务端提供跨设备职业资产、账号、额度、LLM、Embedding 和生产能力

> **2026-09-05 方向更新：** Product Owner 已决定不再以“完整 Career Memory 仅存浏览器、服务端不长期保存履历正文”作为 V3 默认路线。绝对安全无法由纯本地存储保证，尤其云端 LLM / Embedding 仍需处理必要文本；V3 改以“最小暴露、直接身份隔离、语义降敏、用途限制、透明告知和可删除”作为隐私目标。本文第 1 节及 D-036 是当前方向；其余既有 local-first / 无履历持久化章节暂作为上一轮架构探索保留，待用户完成后续 V3 输入后统一重构。任何冲突均以本更新和 D-036 为准，不得继续把旧路线当成默认承诺。

## 1. 核心判断

V3.0.0 从本地单用户产品进入真实市场验证和账号级服务。Career Memory 的价值来自持续积累、
跨任务复用、跨设备恢复以及围绕不同求职目标重新选材，因此默认允许服务端持久化语义降敏
后的职业事实和派生索引；不再为了追求无法证明的“绝对本地安全”牺牲核心产品体验。

- 用户设备优先保存姓名、电话、邮箱、真实学校/公司名称等直接身份，以及把服务端内部实体
  引用恢复为真实名称所必需的映射；
- 服务端保存账号级 Career Memory、事实来源与 revision/hash、派生 Embedding、任务和必要
  的产品状态，用于跨设备、恢复、检索、生成和渐进补充；
- 发送第三方模型时继续只选择完成当前任务所需的最小事实集合，不因为服务端已有完整 Career
  Memory 就把全部职业资料发送给模型；
- “语义降敏”降低直接识别风险，但详细经历组合仍可能重新识别个人，产品必须如实说明，不把
  它宣传为匿名或绝对安全；
- 权限隔离、加密、访问与删除、日志脱敏、备份边界和供应商数据处理分别验收；服务端持久化与
  第三方模型处理不能混成一句“云端处理”；
- V3 小版本以真实用户问题和可验证假设排序，不提前把 V3.1～V3.9 固定为功能清单。

目标体验是：用户正常完成简历、面试和后续求职任务，Career Memory 自然变得更完整；用户
无需感觉自己在维护数据库，也无需为低价值专业参数持续做决定。
...
V3.0.0 每个实际版本仍按全局规则分别记录功能验收与结构变更验收，并绑定具体 commit。以下高风险项目必须由可读取客户端、服务端和部署配置的验收 Agent 独立验收。
