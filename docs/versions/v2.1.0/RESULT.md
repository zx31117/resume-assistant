...
> **V2.1.0 已完成撤回修正并重新公开发布**。功能验收结论保持通过；预检标题已改为版本无关文本，
> 本地统一预检与 GitHub Windows CI 均重新通过，annotated tag `v2.1.0` 已重新创建并推送。
> 当前产品基线：已发布 V2.1.0
...
> 模型 E2E 补证与随后 Product Owner 人工验收的累计结论为准。
...
### H8-R1 Product Owner 人工验收打回与 H8-R2 批准（2026-09-12）

Product Owner 使用 H8-R1 最终 onedir 继续人工验收时观察到：进入 P4 排版过程后，系统短暂弹出
CMD/控制台窗口，窗口立即消失后又重复出现多次。该问题发生在核心生成流程，虽未否定 H8-R1 独立
源码验收对既定 §20 合同的 PASS，但使 H8-R1 无法完成用户体验门禁，因此不得发布。

现有 PLAN 规定 Word COM 使用不可见实例并防止 Word 对话框、挂死或误关用户 Word，但没有明确覆盖
短寿命 CMD/Python/conhost 窗口，也没有要求在最终 onedir 中持续捕获可见窗口事件。由此形成的合同与
验收盲区由 PLAN §21 补齐；Product Owner 已确认按该节形成 H8-R2，不转入 V2.1.1。

H8-R2 当前状态为**待开发**。根因尚未由开发证据确认，不能把“子进程缺少隐藏窗口参数”的合理怀疑
...
### H8-R2 首次独立源码验收（2026-09-12）

#### 1. 身份、独立性与冻结状态

- 验收者声明未参与 H8-R2-SRC、H8-R1 或 H8 的实现、自测、源码修复或 RESULT 编写；
- 验收绑定 H8-R2-SRC `f5124c2af448fc6fa50a599187f643e62a814ff8`；验收前后固定
  `<review-workspace>` 均为该 commit、detached、clean，PLAN blob 均为
  `4fd455c4fcc29b0f315f434a919c3da13d8b5f45`；
- H8-R2-DEV `f9b26eb1a2df7d943a7b571e66482c466562f47d` 的唯一父提交为 H8-R2-SRC，二者之间
  只修改 RESULT；DEV RESULT blob 为 `b1c82ea94dfbb1a7916a42b969611e12bbacdfdb`；
- 动态验证在一次性隔离副本与隔离 runtime 中执行，未修改 review 或候选包。

#### 2. 已独立证明的增量结论

| 验收项 | 独立结果 |
|---|---|
| 旧 H8-R1 包负向 | 以冻结 EXE worker 模式触发真实 P4 Word→PDF 路径，捕获 6 次应用后代 `tasklist.exe` / `PseudoConsoleWindow` 可见窗口 |
| 新 H8-R2 包对照 | 同一监测器、同一 worker 路径下，应用后代可见控制台/Word 窗口为 0 |
| 六组失败矩阵 | `all_ok=true`；COM 缺失、损坏 DOCX、超时、busy、恢复与冻结 worker 均零窗口、零 WINWORD 泄漏 |
| 结构与冻结包 | 生产子进程入口统一无窗口；无 shell/CMD 二次包装；包内 PYZ 三个生产模块检查通过 |
| 确定性回归 | `PASS=22 / FAIL=0`；JD/rewrite 次数、14ms 终态差、锚点、fail-closed 与进程清理通过 |
| 统一预检与包审计 | `precheck.py` 阻断项 0；包审计 4045 文件、170,264,106 字节、标记/违禁命中 0 |
| 前端免跑条件 | H8-R1 与 H8-R2 前端 4/4 文件逐字节一致，按 PLAN §21.4 免跑前端浏览器矩阵成立 |

最终包身份复核一致：`ResumeAssistant.exe` 为 16,753,725 字节，SHA-256
`91e75083367eb028a8e5ddf38c74da5460dece72e0a4caecfcc82ba9b51d68d5`，manifest 4045 行；包内
没有 ReportLab、旧 `pdf_renderer`、测试注入、凭据、开发机绝对路径或 CMD 清理路径。

#### 3. 阻断与未覆盖

验收环境调用真实模型 `doubao-seed-evolving` 时连续返回 `404 ModelNotOpen`，因此
`h8_real_model_e2e.py` 未完成。验收者没有用开发侧既有结果代替本轮证据，也没有把 worker 模式的
P4 对照冒充“启动→导入→生成→P4→预览/下载→退出”的完整真实链。

由此，本轮独立证明了闪窗修复在 P4 worker、失败路径、冻结字节码和前端不变边界内成立，但没有证明
新包在完整真实生成中仍为零窗口，也没有重新取得真实链的 artifact、下载、锚点、JD 次数和计时联合
证据。该缺口属于当前模型授权环境阻断，尚无证据表明是 H8-R2 源码缺陷；候选不变，不创建 H8-R3。

环境恢复后，仍由未参与实现的验收者绑定同一 H8-R2-SRC 和 `release-h8-r2` 包，伴随同一窗口监测器
补跑完整 `h8_real_model_e2e.py`。必须同时得到：operation 成功、应用后代控制台/Word 窗口为 0、
JD=1、rewrite=1、Provider chat=2、P1–P4 时间闭合、锚点绑定、Word/PDF/viewer/download 同源、无
4xx/5xx 和无 WINWORD 泄漏。若只改变本机模型授权或运行时配置，候选身份不变；若需要修改源码、包内
配置或可执行字节，则本结论失效并须形成新候选。

#### 4. 五维结论与下一门禁

- Function：通过（旧包 6 次闪窗、新包同路径 0）；
- Structure：通过；
- Design Fidelity：通过（前端 4/4 字节一致，免跑成立）；
- Integration：部分通过，缺完整真实模型 E2E；
- Release Gate：未通过。

**首次独立验收最终结论：FAIL（未能出具 PASS）。** 阻断项只有真实模型 E2E 未完成。补证通过后，
再由 Product Owner 使用同一冻结包完成一次真实生成并肉眼确认 P4 全程无闪窗；在此之前不得发布。
...
#### 7. Product Owner 人工验收结论（2026-09-12）

Product Owner 使用同一 H8-R2 冻结包完成最终人工复核后，明确反馈：**验收通过**。因此 PLAN §21.4
规定的人工门禁完成，H8-R2 的开发验证、独立验收和 Product Owner 人工验收现已全部通过。

本次确认只改变版本验收状态，不改变 H8-R2-SRC、H8-R2-DEV、PLAN blob 或发布包身份，也不自动等同于
公开发布授权。V2.1.0 当前进入“验收完成、待发布决定”状态；在 Product Owner 明确批准发布前，不更新
公开 main、tag 或发布声明。

#### 8. Product Owner 发布批准与发布前状态（2026-09-12）

Product Owner 已在人工验收通过后另行明确批准发布 V2.1.0。Documentation Agent 据此开始更新
`CURRENT_STATE.md`、根 README、开发文档入口与版本索引，并准备最终发布候选。

发布批准不豁免 PLAN §8.1.8：最终候选必须先在 GitHub Windows CI 上成功运行统一
`scripts/precheck.py`。在该远端门禁取得成功证据前，不创建 `v2.1.0` tag，不把版本写成“已公开发布”。
本节之后形成的发布文档候选将用于触发 CI；CI 成功、annotated tag 创建并推送后，再追加最终发布身份。

发布文档候选的本地统一预检已于 2026-09-12 从头重跑并以 exit 0 完成，全部阻断检查通过。首次运行因
canonical 的 `frontend/node_modules` 缺少 `tsc`/`eslint` 而失败；按锁文件执行 `npm ci` 后复跑恢复绿灯，
...
#### 10. 首次发布撤回与预检标题修正（2026-09-12）

Product Owner 在首次发布后指出统一预检仍显示“V2.0.2 统一预检”，并要求直接撤销后重发。核对确认
`scripts/precheck.py` 的模块说明和运行标题均硬编码 V2.0.2；实际测试内容、APP_VERSION、前端版本和
发行包均为 V2.1.0，但该人类可见标题属于发布一致性缺陷。

原远端与本地 annotated tag `v2.1.0` 已删除；原 tag 目标 `8c7bae3746083962b5896519c7f78c82ddaa6548`
仍完整保留在 main 历史，并保存本地 `refs/backup/v2.1.0/initial-tag-target` 供追溯。此次撤回不否定
H8-R2 产品功能、包或验收结论。

修正采用版本无关标题“Resume Assistant 统一预检（本地与 GitHub 共用入口）”，避免后续版本再次因
硬编码版本号产生相同问题。修正提交必须重新运行本地统一预检并取得 GitHub Windows CI 成功；随后
重新创建 annotated `v2.1.0` tag，并追加新 tag 目标和重发 CI 身份。

修正后的本地 `python scripts/precheck.py` 已从头执行并以 exit 0 完成，标题正确显示为版本无关文本，
全部阻断检查通过。ruff、pip-audit 和完整 ESLint 继续报告既有非阻断基线；npm audit 本次因 registry
连接被重置未取得摘要，仍按既有规则作为报告项，不影响阻断结论。除 `scripts/precheck.py` 两处显示文本
和本轮状态文档外，H8-R2 产品源码、测试逻辑、前端产物与发布包均未改变。

#### 11. V2.1.0 重新发布记录（2026-09-12）

- 标题修正提交：`5d72a2e08ebd4fa416b4b1dcdd79c1d08dfc7cfd`；
- 修改范围：`scripts/precheck.py` 两处显示文本及撤回/重发状态文档；产品源码、测试逻辑、前端产物与
  `release-h8-r2` 发布包不变；
- 本地统一预检：exit 0，标题为“Resume Assistant 统一预检（本地与 GitHub 共用入口）”，阻断项全过；
- GitHub Windows CI：run `34696307213`，绑定 `5d72a2e08ebd4fa416b4b1dcdd79c1d08dfc7cfd`，
  结论 `success`；
- 新 annotated tag 对象：`df8dec9d519a142c8dba7a392a951ebc3afe72f8`；
- 新 `v2.1.0` tag 目标：`5d72a2e08ebd4fa416b4b1dcdd79c1d08dfc7cfd`，已推送至 GitHub；
- 原 tag 目标 `8c7bae3746083962b5896519c7f78c82ddaa6548` 只作为首次发布历史保留，不再是当前
  `v2.1.0` 发布身份。

V2.1.0 撤回重发至此完成。当前公开版本以重新创建的 annotated tag 为准；后续修改进入 V2.1.1，
不得继续移动或覆盖该 tag。
