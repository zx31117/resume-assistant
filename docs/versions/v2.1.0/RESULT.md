...
| T10 RESULT 与冻结候选 | 已失效，待新候选 | 原交接 H 仅保留历史证据；完成 T12-R1 至 R5 后冻结新 clean 候选 H2 |
...
| T12-R1 至 R6 | 未开始 | 等待开发 Agent 接收集中返工契约 |
...
2. **生成阶段与明细纵向堆叠**：四阶段总览在上、全部真实 operation 明细在下，用户必须滚动且
   同时面对过多技术信息。要求改为左右结构：左侧四阶段，右侧仅当前高层阶段明细；每进入一个
   新阶段，右侧整体切换一次，不累计展开历史明细。
...
- 开发 Agent 在固定 `<current-workspace>` 完成 T12-R1 至 R5，更新本 RESULT 的实施、反向退出、
