...
1. **无简历初始页**：结构与 `DS-002/previews/welcome-theme-A.png` 一致；左侧整张“我有一份
   ...
   点击，不进入演示或产生假状态。
2. **上传解析页**：采用原型的左右布局。左侧展示文件解析完整流程和真实状态，右侧流式显示当前
   高层阶段的用户可理解事件；阶段推进时右侧清空并切换，不累计全量内部 operation。
3. **身份/JD 页**：左侧按原型保留身份摘要、目标岗位与 JD 输入、真实分析结果；右侧按原型保留
   生成前检查、产品说明和生成动作。V2.1.0 已固定模板，模板徽标、下拉框和用户选择全部退出。
4. **生成处理页**：继续使用与上传解析相同的左右视觉语法；左侧为完整四阶段，右侧为当前阶段
   流式输出。进入 processing 后不再显示身份/JD 输入摘要。
