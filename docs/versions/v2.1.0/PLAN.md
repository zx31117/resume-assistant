...
- 右侧只流式追加当前高层阶段的用户可理解事件；进入下一阶段时清空旧阶段明细并切换一次，
  不把全部 operation 历史无限累积在页面；
...
- 右侧进入下一高层阶段时整体切换，不累计展示所有历史 operation；左侧保留已完成状态；
...
| T12-R2 | 还原上传解析页 | 左侧完整解析流程，右侧当前阶段流式输出；跨阶段清空切换；D-038 确认和错误恢复无回退 |
| T12-R3 | 还原身份/JD 页面并退出模板选择 | 左侧身份+JD 按原型结构；右侧检查/说明/生成结构；模板徽标、下拉和用户选择状态计数为 0 |
| T12-R4 | 还原生成处理页 | 左侧完整四阶段，右侧当前阶段流；真实阶段边界切换；processing DOM 无重复身份/JD 摘要和全量 operation 列表 |
