...
### 20.10 90 分钟演示应急轨与风险预案（2026-09-09）

Product Owner 明确当前仅剩约 90 分钟，目标是尽量形成一份可展示的新包，同时保留刚刚实际跑通的
旧包作为兜底。§20.1–§20.9 是正式 H8 的完整发布门禁，范围正确但不应被伪装成 90 分钟内必然能够
全部完成。为防止再次发生“开发未跑完→把工作推给验收→仍命名候选”，本轮分为两条互不混淆的轨道：

- **正式 H8 轨**：继续满足 §20.1–§20.9 全部开发、自测、独立验收和 Product Owner 门禁；
- **演示应急轨**：允许形成具名 `DEMO-HOTFIX-20260909` 包，只用于本次现场展示，不得命名 H8、
  不得更新 CURRENT_STATE、main、tag、Release 或对外宣称问题已经完整解决。

演示兜底已冻结在 `D:\demo\resume-assistant\X`。该目录保存复制时正由 8000 端口运行的 onedir，
不包含用户 runtime、履历或 API Key；新开发不得覆盖 X，不得用新候选迁移/修改 X 所依赖的默认
runtime。所有热修开发和测试必须使用新的隔离 `RESUME_DATA_DIR`。

#### A. 90 分钟内最可能发生的风险与预案

| 风险 | 触发信号 | 演示应急预案 | 正式 H8 后续要求 |
|---|---|---|---|
| Word COM 在源码可用、打包后不可用 | pywin32 hidden import/DLL 缺失、COM 未初始化、Office 位数或权限异常 | 前 20 分钟先做最小 onedir/打包探针；不能导出即停止集成，不修 ReportLab，保留 X | 补齐 spec、COM apartment、环境 fingerprint 和缺失负向 |
| Word 弹窗、挂死或误关用户 Word | 不可见对话框、转换超时、已有 Word 被复用 | 使用独立转换子进程/实例、单任务互斥和短超时；只能清理自有进程；出现一次不可控挂死即停止热修 | 完成超时、清理、并发、宏/外链关闭和用户进程保护矩阵 |
| Word→PDF 成功但 PreviewAnchor 失效 | PDF 文本被拆 span、中文匹配失败、点击高亮错位 | 禁止沿用 ReportLab 坐标；应急包可诚实关闭坐标高亮，仅保留可查看的依据列表 | 正常 fixture 全锚点重建，失败项显式 unavailable |
| 新 PDF 路径重新引入 404/405 | viewer 能看但下载失败，或 Range/GET 方法不一致 | 复用一个已落盘 URL；只要预览/下载任一 4xx 就判定热修包不可展示，切换 X | 完成 §19.5/§20.4 全协议矩阵 |
| DOCX 被覆盖或 PDF 对错 revision | 固定文件名、重复生成后预览旧 PDF、hash 不一致 | 应急包至少把 DOCX/PDF 文件名绑定 operation；重复生成一次核对结果确已切换 | 建立完整 result_revision_id 与原子 artifact 对 |
| 重复 JD 分析仍存在 | Network 中先 `/jd/analyze`，生成内又出现 JD LLM | 优先删除普通输入页自动 LLM 调用；一轮生成计数不等于 1 即不使用新包 | 后端可信分析 artifact 复用属于未来方案，当前保持单次分析 |
| 计时修正扩成后端 API 大重构 | 需要同时改大量 schema/历史诊断/系统后台 | 应急包可做最小真实修正：补齐阶段映射并加入活动 `stage_elapsed_ms`；若仍不能保证准确，隐藏阶段秒数，只保留真实总用时和状态，禁止继续显示错误数字 | 正式 H8 仍须以后端用户阶段投影和终态 ≤250ms 为准 |
| 修复一个问题引发白屏/Hook 回归 | Console 出现 #310、Hook order、uncaught error | 不继续现场补丁叠加；立即放弃热修包并使用 X | H6 runner 与 ErrorBoundary 矩阵全部重跑 |
| 完整回归/onedir 超时 | 到硬检查点仍在跑、日志无终态、包未实际启动 | 不把“运行中”写成通过；停止任务，保留日志，切换 X | 全部 mandatory 项完成后才能命名 H8 |
| 新版本污染现有演示数据 | 默认 runtime 被迁移、Key/履历变化 | 热修全过程强制隔离 runtime；X 只在最终回退演练时使用默认 runtime | 正式候选继续执行 runtime 隔离与迁移门禁 |

#### B. 演示热修的最小范围

在 90 分钟限制下，只允许修改以下内容：

1. ReportLab 产品调用替换为“最终 DOCX→本机 Word→PDF”，转换失败 fail closed；
2. viewer 与 PDF 下载读取同一已落盘 PDF，正常演示路径零 404/405；
3. 删除普通用户输入页的重复 JD LLM 调用；
4. 修正活动阶段实时耗时和未映射时间；若无法在时间内证明准确，则删除阶段数值而不是显示错误时间；
5. 与上述改动直接相关的依赖、spec、测试和固定错误态。

不得在应急轨顺带重构全局状态、视觉系统、历史诊断页、未来流式内容、单 Fact 重生成、服务器转换器、
模板排版优化或其他需求池项目。PreviewAnchor 若无法在 Word PDF 上及时可靠重建，只能诚实降级，不得
保留错误高亮。

#### C. 硬检查点与停止规则

从 Development Agent 实际开始计时：

| 时间点 | 必须得到的可观察结果 | 未达到时的动作 |
|---|---|---|
| T+20 分钟 | 最小脚本能让本机 Word 把固定 DOCX 转成可打开 PDF；进程可退出 | 停止 Word 集成尝试，保留 X，不再修 ReportLab |
| T+40 分钟 | 产品源码能生成 DOCX→PDF；独立打开 PDF 与 Word 视觉一致，无挂死 | 若仍在处理 COM/字体/路径问题，停止形成新演示包，转 X |
| T+55 分钟 | 一轮真实生成仅一次 JD 分析；总时间和活动阶段时间不说谎 | 计时未闭合则隐藏阶段秒数；重复 JD 未消除则新包不展示 |
| T+70 分钟 | 源码运行完成一轮真实 JD：无白屏，Word/PDF/预览可用，零 404/405 | 任一 P0 失败即停止，转 X |
| T+82 分钟 | 新 onedir 已构建并从隔离 runtime 实际启动、完成最短冒烟 | 未实际启动的包不得带到现场，转 X |
| T+90 分钟 | Product Owner 在新包完成一次关键路径确认 | 未明确通过则使用 X，不再临场修改 |

任何检查点一旦决定回退，不得为了“已经投入很多时间”继续占用演示准备窗口。未完成的源码和日志保留
供演示后正式 H8 使用，但不得覆盖 X 或冒充可展示版本。

#### D. 演示热修最低通过条件

`DEMO-HOTFIX-20260909` 只有同时满足以下条件才可优先于 X：

- 使用隔离 runtime 冷启动成功，首页、工作台和后台配置页可进入；
- 使用演示 JD 完成一轮生成，页面无白屏，Console 无 React Hook/#310/uncaught error；
- Word 下载可打开；PDF 确由该 Word artifact 转换，独立 PDF 与 Word 关键排版一致；
- PDF.js 能显示同一 PDF，点击下载 Word/PDF 均成功，正常链路 404/405 为 0；
- 一轮生成 JD LLM 调用恰好 1；总用时真实。阶段秒数只有在实时且可解释时才显示；
- 重复生成一次不会串用旧 artifact，转换与生成次数符合 operation 数；
- 新包完整复制到新的具名目录，记录 exe hash、文件数、来源提交和已知未完成项；
- Product Owner 明确选择新热修包。否则默认使用 X。

演示结束后，热修包的通过不等同正式验收；Development Agent 必须把有效改动整理到 clean H8 开发链，
完成 §20.8 的全部专项矩阵、回归和 onedir 门禁，再由 Documentation Agent、独立 Acceptance Agent
和 Product Owner 依次推进。

## 21. Product Owner 第六次返工：排版阶段零控制台闪窗（2026-09-12）

### 21.1 触发事实、优先级与范围

H8-R1-SRC 已通过独立源码验收，但 Product Owner 在最终 onedir 人工验收中观察到：进入 P4 排版阶段
后，系统反复短暂弹出 CMD/控制台窗口，窗口出现后立即消失并再次出现。该行为发生在普通用户核心
生成链，破坏桌面产品完整性，判定为 **P1 人工验收阻断**；H8-R1 不得发布，须形成 H8-R2。

本轮只处理生产生成链的可见进程窗口及其直接回归。V2.1.1 的跨页面状态保持、刷新/重启恢复，以及
新的视觉、文案、排版或业务能力均不进入本轮。

### 21.2 先定位再修复

Development Agent 必须先在未修改的 H8-R1 最终包上稳定复现，并连续捕获应用进程树中的进程创建与
顶层窗口显示事件，至少记录时间、映像名、父/子 PID、窗口类别、对应 P4 步骤及脱敏后的启动方式。
不得记录 API Key、完整 JD、履历或正文。必须把每次闪窗映射到确切生产调用点，并区分：转换 worker、
能力探针、CMD/PowerShell 包装、Word COM/提示框及其他子进程。不能以推测直接修改一处后结束审计。

若实际根因不是生产子进程窗口，或消除闪窗需要改变 DOCX→PDF 技术路线、进程隔离/超时语义或用户
可见流程，立即停止并请求 PLAN 修订；不得用缩短进程时间、遮挡窗口或降低检测频率伪装修复。

### 21.3 修复不变量

1. Windows 最终 onedir 从启动到生成完成，不得显示任何由应用及其后代创建的 CMD、PowerShell、
   Python console、conhost 或 Word 窗口/提示框；应用主窗口不计；
2. 所有生产子进程入口采用统一、可审计的隐藏窗口策略；不得以 `shell=True` 或 CMD/PowerShell
   二次包装启动 worker。具体 API 由开发决定，但源码态、冻结包和异常路径必须一致；
3. Word COM 继续使用本次独立、不可见实例并禁止交互提示；超时和失败只能清理本次拥有的 worker /
   WINWORD，不得复用、隐藏或结束用户原有 Word；
4. 不改变 DOCX 排版真源、Word→PDF、PDF.js/download 同 artifact、原子发布、锚点、阶段计时、
   单次 JD 分析和 fail-closed 语义；
5. 不因隐藏窗口吞掉 stderr、退出码、超时、崩溃或清理失败；诊断必须继续进入受控日志和错误结果。

### 21.4 开发 Gate

全部通过后才可冻结 H8-R2-SRC：

- **旧包负向**：同一监测器在 H8-R1 最终包的 P4 阶段捕获至少一次与人工现象一致的可见窗口事件，
  证明检测器能发现问题；若无法复现，须由 Product Owner 再复现或提供可绑定证据，不得以零命中
  直接证明新包正确；
- **新包正向**：最终 onedir 完整真实生成期间，应用进程树的可见控制台/Word 窗口事件为 0；启动、
  能力检查、DOCX 构建、Word→PDF、锚点、下载与退出全程纳入监测；
- **失败矩阵**：Word/COM 缺失、损坏 DOCX、超时、并发 busy、恢复后再生成五类路径均为零可见窗口，
  错误可诊断、PDF fail closed、Word 可按原合同下载、无 worker/WINWORD 泄漏；
- **核心回归**：`precheck.py`、H8 确定性矩阵、真实模型 onedir E2E 和包审计全部通过；Word/PDF/
  viewer/download hash、JD 次数、四阶段终态差和锚点仍符合 §20。若前端产品字节发生变化，必须另行
  重跑 H8-R1 真实输入回归和 H6 browser `--all`；前端字节完全不变时可用逐文件 hash 证明免跑理由；
- **人工复核**：Product Owner 使用新冻结包完成一次真实生成，肉眼确认 P4 全程没有闪窗。

窗口检测必须覆盖短寿命窗口，使用进程/窗口事件或足够可靠的连续捕获；单张截图、低频轮询、开发环境
运行或“本机没看到”不构成机器证据。

### 21.5 交付与独立验收

Development Agent 在 RESULT 新增 H8-R2 记录：旧包复现证据、根因与全部生产启动点、修改范围、窗口
监测方法及零事件结果、失败矩阵、回归命令/退出码、前端是否字节变化、最终包路径/文件数/字节数/
EXE hash/manifest、已知偏差和待独立验收问题。先冻结 clean H8-R2-SRC，再形成唯一父为 H8-R2-SRC
且只改 RESULT 的 H8-R2-DEV；不得修改本 PLAN。

Documentation Agent 只做身份与 RESULT 语义交接。独立 Acceptance Agent 绑定 H8-R2-SRC，在隔离
副本和 runtime 中独立复现旧包、监测新包正常与失败路径，并核对 §21.3–§21.4；不能仅凭源码存在
隐藏窗口参数出具通过。独立验收与 Product Owner 人工复核全部通过前，不更新 `CURRENT_STATE.md`、
根 README、main、tag 或发布声明。
