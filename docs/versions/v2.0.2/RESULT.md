...
> 当前状态：**已验收**
> 当前阶段：集中返工源码候选 `eb4bd30` 的 T8 功能与结构验收均通过，人工确认与文档收口完成；用户已批准发布，annotated tag `v2.0.2` 已发布
> 计划日期：2026-09-05
> 批准日期：2026-09-05
...
- **发布状态：已发布。** 用户明确批准后，文档 Agent 核对远端 `publish/main` 未漂移、候选可快进且 `v2.0.2` tag 不存在，并原子推送发布候选与 annotated tag。最终发布 commit 不在自身文档中回填自身 SHA，发布身份见 §12。

**最终文档结论：V2.0.2 已验收并发布。**

## 12. 发布后补录（2026-09-05）

- 用户明确批准发布最终候选 `78bb909c18ca28e45b54406536aa326887caa1ca`。
- 发布前联网 fetch 证实远端 `publish/main` 仍为预期 V2.0.1 基线 `f4e69aae0577ea723e9ef5427b20287add76d06c`，候选是其可快进后代，远端 `v2.0.2` tag 不存在。
- 文档 Agent 创建 annotated tag `v2.0.2`，tag object 为 `f709d2b4abbf5ac351ac98bd572b07ddb0ae1ed4`，peeled commit 为 `78bb909c18ca28e45b54406536aa326887caa1ca`；远端 `main` 与 tag 使用原子推送发布成功。
- 推送后通过远端对象查询确认：当时远端 `main` 与 `v2.0.2^{}` 均解析到 `78bb909c18ca28e45b54406536aa326887caa1ca`。本附录属于 tag 之后的发布事实记录，不移动已经正确发布的 `v2.0.2` tag。
