...
| HEAD commit（短） | `14b49ac`（V1.3 初始实现提交；第三轮最终验收内容仍位于该分支未提交工作区） |
...
```
 ...
 M docs/versions/v1.2/PLAN.md
 M docs/versions/v1.3/PLAN.md
 M docs/versions/v1.3/RESULT.md
...
| --- | --- | --- | --- | --- |
| T1 | V1.2.1 Baseline | 保存 Experience CRUD、RAG、模板、下载和关键内容结果 | 保留 V1.2.1 既有 `CRUD / rag_service / template_renderer(v1.2 layout) / download route`；新增 V1.3 接口不破坏 V1.2 baseline 路径 | T / C |
...
- **新增核心接口**：`POST /api/resume/generate-docx`
  ...
  - 其中 `build_meta`、`render_stats` 为 V1.3 Result 新增诊断（见 `api/schemas.py:BuildCounts / BuildMeta / RenderStats`）
- **统一错误响应**：新增 exception_handler(`DomainError`)，映射到 `DomainErrorOut`（ok=false / error_code / stage / message / retryable / details）。
- **旧接口**：`POST /api/resume/generate`（V1.2 baseline）——无破坏性变化，保留但在路由与 docs 中标记 deprecated。
...
| `models.Experience` | 无 | V1.2 字段保留（type/company/title/role/time/description/achievements/skills 等） |
| `models.User` | 无 | |
...
| 文件 | 变化 |
| --- | --- |
| `core/errors.py` | 调整 `VectorIndexNotReadyError.__init__` 入参（`failed_ids / pending_ids` 而非 `details` keyword）；其它 DomainError 子类保持 V1.3 约定语义。 |
...
| `services/llm_service.py` | strict 模式：N 次 Pydantic 结构化解析仍失败抛 `LLMOutputInvalidError`，不返回默认空模型（避免 V1.2 时代“空成功”污染 Builder）。 |
...
| `_v13_validation.py`（新增，验收脚本） | 覆盖 PLAN §8.2 必测 1-10：真实数据 E2E → T1~T10 逐项 assert → 输出 `backend/output/V1.3_§8.2_验证表.json` + stdout 人类可读汇总。 |
...
**产品规则（PLAN §3.3）**：AI 只能生成已有经历的 bullets；V1.3 不生成或渲染个人总结/自我评价。
...
- V1.3 **不生成、不渲染**个人总结/自我评价（PLAN §3.2）
...
> 验证表 JSON 产物：`backend/output/V1.3_§8.2_验证表.json`（RESULT 归档时一并保留）
...
| R5 | RESULT 未完整提供“实际全局变化分类表”与“验证表” | **已解决（本文 §二 + §四）** | §二 分类覆盖 API / 数据模型 / 模块职责 / 配置依赖（无变化项写“无”）；§四 以 PLAN §8.2 为骨架，每项给脚本 key / 状态 / 证据，均选自 `V1.3_§8.2_验证表.json` |
...
> - `CURRENT_STATE.md`：当前 V1.3 已交付能力 / 未交付项
> - `README.md`：V1.3 新接口的调用方式（请求/响应结构 + 典型错误）
> - `DECISIONS.md`：长期产品规则已经记录；不得把旧实现的 `profile_source` 三层合并误记为长期决策

1. **V1.3 核心路径**：`POST /api/resume/generate-docx` 已能在单次调用中完成 `index_check → JD 分析 → RAG 匹配 → SQL 回读 → AI 内容生成(strict) → Builder 合并/裁剪 → Renderer 渲染 → 本地存 DOCX`，返回 `ResumeDocxGenerateResponse`（含 stages / build_meta / render_stats）。
2. **Renderer 事实保护行规则（长期）**：item_block 第一行（ItemTitle 行）即使占位符全空也不做“整段删除”，只写 warning 并保留已克隆行；否则 Project/Education 的事实字段会因 start_time/end_time/name/school 全空被隐式丢弃，破坏事实保护。（对应 V1.3 Result 返工根因：R2）
...
| V1.3 完整流程验收报告（txt） | `backend/output/验收报告_V1.3_完整流程.txt` | 由更早一次 E2E 生成（阶段耗时 8 stages，内容核对 8/8 PASS）；不包含最新 Renderer 修复结果（最新见验证表） |
| §8.2 验证表（自动化 JSON） | `backend/output/V1.3_§8.2_验证表.json` | 最新一次验证产物（10/10 通过，对应本文件 §四） |
...
| mock 数据填充验收 | `backend/output/验收报告_用户数据填充.txt` | V1.2 基线 `fill_user_data.py`（21/21 PASS，baseline 未回退） |
...
**结论：V1.3 已验收，V1 核心链路收口完成。** 后续需求应从 V1.3 的已验收状态建立新版本 PLAN。

> Result 最终交付前请保留：`_v13_validation.py` + `V1.3_§8.2_验证表.json`，不建议归档时删除（便于重复验收）。
