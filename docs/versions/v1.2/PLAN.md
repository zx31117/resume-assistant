# AI Career Resume Assistant V1.2 技术方案

> 文档角色：V1.2 历史执行计划  
> 状态：已实施；路径 A 结果见 [RESULT.md](./RESULT.md)  
> 基线：[V1.1 RESULT](../v1.1/RESULT.md)  
> 经验阅读：必须同时理解“任意 Word 路线被否决”和“标准模板路线”两次决策语境  
> 当前全局上下文：[项目总览](../../README.md) · [决策记录](../../DECISIONS.md)；本文本身保留 V1.2 当时语境

> 版本：V1.2（标准化模板方向）
> 更新日期：2026-08-14
> 前序结论：V1.2 原「解析任意用户上传 Word 模板」方向经实测验证不可行，转向「系统内置标准化模板 + 占位符替换」。
...
V1.2 原方案选择「用户上传 Word → 解析文本框 → 识别章节 → 按坐标写入」。基于 `input/模板.docx` 的真实 XML 结构实测（诊断报告已于 V1.2 阶段清理，下述结论留档），发现以下**不可逾越**的问题：
...
**根本判断**：解析「任意 Word 模板」等价于做 Word 渲染器的子集，工作量和风险指数级膨胀。V1.2 不做这个方向。
...
└──────────────┬─────────────────┬───────────────────┬──────────┬───────────┘
               ...
               └─────────────────┴───────────────────┴──────────┘
                                 ▼
                    ┌──────────────────────────┐
                    │    ResumeContent (V1.1)  │  ← 原始生成结果（字符串/JSON）
                    └───────────┬──────────────┘
                                ▼
┌───────────────────────────────────────────────────────────────────────────┐
│                          V1.2 新增层                                       │
...
数据流关键约束：
- V1.1 输出的 `ResumeContent` **不能**直接喂给渲染器，必须经 `ResumeBuilder` 转成强类型 `ResumeDocument`
...
├── services/
│   ├── resume_builder.py       # 既有，重构：V1.1 输出 → ResumeDocument（裁剪前移）
...
class Profile:
    ...
    summary: Optional[str]    # 自我评价 / 个人简介（V1.1 的 summary 字段）
...
class ResumeBuilder:
    ...
    def build(
        ...
        jd_analysis: dict,       # V1.1 的 JD 分析结果
        generated_content: dict, # V1.1 的 AI 生成内容
...
**V1.2 PDF 布局复刻更新**：参照 `input/用户上传PDF.pdf` 提取的真实布局参数（pdfplumber 逐项测量），不再是 `input/模板.docx`。用纯段落 + 命名样式实现，关键排版特征：
...
- **V1.2 布局参数全部来自 PDF 实测**，非主观设计：页边距/字号/颜色/照片框位置/章节顺序均有 [布局一致性核对脚本](../../../backend/) 46/46 PASS 验证
...
**V1.2 设计原则（用户审核反馈）**：JSON **不做过重 DSL**，第一版只保留 4 个核心字段：`section / style / placeholder / repeat`。`join` / `skip_if_empty` / `layout_rules` 等过滤器与规则**后置到 V2 渐进加入**，避免 MVP 阶段提前设计未验证的复杂度。
...
> 注：V1.2 渲染器对列表字段（如 `skill.items`）的字符串化在代码内固定为「顿号分隔」，不暴露为 JSON 语法。等 V2 有真实多模板需求再升级为 `join` filter。

### 6.5 模板 JSON 关键语法（V1.2 仅 4 个）
...
**V1.2 不支持的语法（明确后置）**：
- ❌ `skip_if_empty`：V1.2 改为代码内固定规则——字段为空时整段删除（不占空白行），不暴露为 JSON 配置
- ❌ `join` filter：列表字段在代码内固定分隔符
- ❌ `layout_rules`：排版规则**后置**，V1.2 渲染器内置固定四级降级（见 §8.2），不读 JSON 规则
...
**为什么不按文本、不全文扫 `{{`**：
- 文本定位的坑：用户在 Word 里把"教育背景"改成"教育经历"→ 旧方案渲染失败。V1.2 靠 style 定位，文本可自由修改
...
**V1.2 空值规则（代码内固定，不暴露为 JSON 配置）**：
- 任意占位符解析为空字符串 → 整段删除（不占空白行）
- 不区分"必填占位符"和"可选占位符"——空了就删段，由 ResumeBuilder 保证必填字段非空（见 §9 缺失直接报错）

### 7.4 clone_paragraph + fill_placeholders（V1.2 简化版）

用户审核反馈明确指出：V1.2 **不主动做 token 拆分 + 动态 Run 复制**，因为 python-docx 对 Run XML 操作复杂度高，且 V1.2 的优势正是"控制模板"——既然模板由我们标准化，就让模板阶段保证「一个占位符独占一个 Run」，渲染时只替换 `w:t` 文本、保留原 Run 属性即可。Run 级 token 拆分算法留 V2。
...
4. 多占位符混排的复杂场景（如 `{{work.company}} | {{work.role}}` 这种 1 个 Run 里塞两个占位符的情况）→ V1.2 模板编写规范**禁止**，由模板作者拆成多个 Run；渲染器遇到时降级为"字符串拼接替换"，不保证字体分角色继承

**为什么不做到 Run 级动态拆分**：
- python-docx 对 Run XML 操作复杂度高，V1.2 MVP 不上
...
**用户审核反馈明确指出**：旧版 §8.2 的 `{"action":"reduce", "target":"item_count"}` 与 §8.1「禁止删除用户经历条目」**自相矛盾**（item_count 减少 = 删除经历）。V1.2 删除该能力，LayoutOptimizer 只调段落级样式，不裁条目。

### 8.2 排版规则引擎（LayoutOptimizer，V1.2 固定四级降级）

输入：已填充完成的 Document + 模板 JSON 的 `layout.page_limit`。

V1.2 排版规则**代码内固定**，不读 JSON 规则（layout_rules 后置到 V2）。算法：
...
class ProfileResolver:
    ...
    def resolve(
        ...
        resume_content_profile: dict,       # V1.1 AI 生成结果里的 profile（JD 定制化求职意向等）
    ) -> Profile:
        """
        优先级（字段级合并，非整体替换）：
...
### 10.1 模板注册（管理用，V1.2 先本地文件不做 HTTP 上传）
...
V1.2 简化：模板直接放 `backend/templates/` 目录 + 在 `config/template_mapping.json` 里登记，无需 HTTP 上传。
...
| V3 | Word → 模板自动转换 | 用户上传一份简历 Word → AI 识别样式 → 自动生成标准化模板 + JSON（**这就是旧方案的最终形态，但放在 V3 做而不是 V1.2 硬上**） |
...
- 把 3 个致命复杂度（文本框家族识别 / 坐标去重 / 章节锚点识别）**整体移出了 V1.2 范围**，移到 V3 作为独立能力
...
- [V1.2 RESULT](./RESULT.md)
- [跨版本决策记录 D-011](../../DECISIONS.md#d-011-模板视觉参照改用用户-pdf)

重构前原文曾单独保存，现已由项目维护者清理；本 PLAN 与对应 RESULT 作为 V1.2 的保留记录。
