...
> 当时后续设想：V1.3（功能演进版，含 GitHub 发布计划）；该设想后来被当前 V1.3 核心链路收口计划替代
...
**是**：
- V1.2.x 分支上的**代码质量与可维护性修复版本**
- 不引入任何新功能、不改动业务行为、不破坏 API 契约
- 为后续 V1.3（功能演进 + 开源发布）扫清代码层面的历史包袱

**不是**：
- ❌ 不是 V2（不做多用户、不做新模板语法、不做 Chroma 升级）
- ❌ 不碰 Embedding / LangChain 相关链路（纯向量模型不可用 + LangChain 与豆包接口不兼容，两条问题已明确接受现状，**不在 V1.2.x 范围内修复**）
- ❌ 不做任何功能体验优化（技能匹配分偏低、JD 分析超时等性能/质量问题，留到 V2）

### 0.2 不修复清单（避免范围蔓延）

以下问题在 V1.0/V1.1 验收报告中已标记为 "⚠️ 暂缓"，在 V1.2.1 中**依然不修**，留待 V2：
...
```
V1.0 → V1.1 → V1.2 → V1.2.1（历史遗留清理）→ V1.3（功能演进 + 开源发布）→ V2.0（架构升级）
```

- V1.0：初始功能版本
- V1.1：迭代修复
- V1.2 / V1.2.0：PDF 布局复刻完成，验收通过（本版本前序）
- V1.2.1：历史遗留清理版（本版本，纯代码清理，不改业务行为）
- V1.3：功能演进版（能力扩展 + GitHub 发布准备）
- V2.0：功能演进（多用户、新模板语法、AI 链路重构等）

> 版本边界说明：本版本严格限定为 V1.2.1，不与 V1.3 的 Git 历史清理工作混淆。V1.2.1 只动源码与配置文件，不执行任何 `git filter-branch` / `git rm --cached`。
...
> 注：T2 的「已入库文件 git rm --cached」+ Git 历史清理步骤，具体执行放在 **V1.3**（因为需要配合 `git filter-branch` 一起做，见后续 V1.3 方案），V1.2.1 只写 `.gitignore` 文件本身。
...
**目标**：先获取 V1.2 已验收环境的真实版本（pip freeze / pip list），锁定当前已验证版本，确保环境可重现。V1.2.1 不做依赖升级策略设计，V2 再做依赖升级。
...
**前置条件**：
- §1 Baseline 已建立。
- V1.2 已验收环境的 Python 解释器与依赖可访问（可在该环境执行 `pip freeze` / `pip list`）。

**执行动作**：
1. **环境事实优先**：在 V1.2 已验收通过的环境中执行 `pip freeze`（或 `pip list --format=freeze`），导出当前已验证可用的真实版本。
...
5. 在 `requirements.txt` 中以注释说明每条约束的来源（如 `# pydantic: 锁定 V1.2 验证环境实际版本`、`# 不在 V1.2.1 做升级，留待 V2`）。
6. **不引入**依赖升级设计文档、不写「建议升级到 pydantic 2.x」之类的迁移说明（这是 V2 的工作）。

> 策略说明：V1.2.1 的目标是「确定并锁定已验证环境版本」，让推 GitHub 后任何人在干净 venv 中 `pip install -r requirements.txt` 都能复现 V1.2 的运行环境。依赖升级与版本演进统一在 V2 处理。
...
**失败处理**：
- 干净 venv 安装失败（版本冲突）→ 回到 V1.2 验证环境，再次确认 `pip freeze` 输出，修正钉版条目；不要为了通过安装而放宽到无版本约束。
- 某依赖在干净 venv 中实际安装版本与 V1.2 环境不一致（如 PyPI 已无该版本）→ 记录差异，寻找最接近的可用版本钉版，并在注释中说明。
...
2. 四层验证全部通过后，删除以下 4 个函数体：
   ...
   - `apply_keyword_bold()`（L202 附近）—— V1.2 明确取消关键词加粗功能
...
5. 文件顶部 docstring 改为：「V1.2 主用：TemplateSpec / SectionSpec / RowSpec / CellSpec / LayoutSpec；旧路线类已在 V1.2.1 清理」。
...
通过标准：
- 脚本退出码 0
- 生成 1 个 docx（`resume_user_mock.docx`）+ 1 份诊断报告 txt（V1.2 的原有产物路径不变，只是不在 git 里追踪）
...
| T3 | 钉版后某依赖在干净 venv 中实际安装版本与 V1.2 环境不一致，或 PyPI 已无该版本 | 中 | 中（pip install 失败） | 以 V1.2 验证环境 `pip freeze` 为准；冲突时记录差异，寻找最接近可用版本；先在干净 venv 验证一次 |
...
## 6. 当时计划的后续衔接（已被当前 V1.3 重新定义）

> 历史状态说明：以下内容是 V1.2.1 编写和验收时对下一版本的设想，当时准备把 V1.3 用于 Git 历史清理和 GitHub 发布。后续架构审核发现 JD 驱动 DOCX 主链路仍未闭环，用户重新明确 V1/V2/V3 边界，因此当前 V1.3 改为核心链路收口。原设想保留用于回顾和学习决策过程，但不再是当前开发指令。当前计划见 [../v1.3/PLAN.md](../v1.3/PLAN.md)。

V1.2.1 只是「代码层」清理，**不碰 Git 历史**。V1.3 将完成：
...
```
V1.2.1 PASS
[ ] 版本边界正确（V1.2.1，前序 V1.2.0/V1.2，后续 V1.3）
...
下一步：用户确认本方案后，按 §3 顺序进入开发，完成后执行 §4 验收并产出 [RESULT.md](./RESULT.md)。当时计划验收后进入 GitHub 发布准备；该计划后来被当前 V1.3 核心链路收口方案替代。
