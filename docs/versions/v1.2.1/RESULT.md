...
> 阅读提示：本文保留了当时对下一版的 GitHub 发布设想，该设想后来被当前 V1.3.0 重新定义所替代

> 版本：V1.2.1（历史遗留清理版）
> 前序版本：V1.2.0 / V1.2.0（PDF 布局复刻完成，验收通过）
> 当时后续设想：V1.3.0（功能演进版，含 GitHub 发布计划）；当前 V1.3.0 已重新定义为核心链路收口
> 验收日期：2026-08-15
...
> **历史背景回应**（对应用户指出的 V1.0.0/V1.1.0 报告中那条「向量库 Chroma PersistentClient → numpy+JSON 文件 补装 VC++ 即切回 ❌ 未切」）：
>
> V1.0.0 时因 Windows 环境缺 VC++ 运行库 / onnxruntime DLL 加载失败，导致 Chroma 默认 embedding 路径无法使用，只能回退 numpy 余弦 + `vectors.json`。本次 V1.2.1 明确验证：**环境已就绪，已切回 Chroma 原生后端**，且通过「全部显式传 embeddings / query_embeddings」完全绕开 Chroma 默认 onnx embedding 路径，从根因上避免了 DLL 崩溃；同时提供了 numpy→chroma 迁移运维接口，对已存在的 `vectors.json` 历史数据可一键安全迁移。
...
```
V1.2.1 PASS
[x] 版本边界正确（当前 V1.2.1，前序 V1.2.0，后续 V1.3.0）
[x] Baseline 已建立
[x] PII 源码 = 0
[x] PII 运行产物 = 0
...
[x] 死代码无调用方（四层验证）
[x] 无意外 API 变化
[x] Chroma 原生后端已切回（backend=chroma，非 numpy 回退；V1.0.0 ❌未切 → V1.2.1 ✅已切回）
...
## 5. 当时计划的后续衔接（已被当前 V1.3.0 重新定义）

> 以下五项是 V1.2.1 验收当时记录的下一步设想，作为历史保留。当前活动计划以 [../v1.3.0/PLAN.md](../v1.3.0/PLAN.md) 为准。

V1.2.1 已完成所有 8 个 Task（T1-T8），满足 V1.3.0 前序条件：
- T1 PII 清理 → V1.3.0 G1/G2 历史层 PII 清理的基础
- T2 .gitignore 重构 → V1.3.0 G2 Index 层清理的基础（已验证 Chroma 全部产物被忽略）
- T3 requirements 锁定 → V1.3.0 G3 干净 venv 可移植性验证的基础（含 chromadb==1.5.9）
- T4-T7 代码清理 → V1.3.0 无需再处理代码层问题
- T8 Chroma 专项 → V1.3.0 可确认向量库状态、忽略覆盖、初始化鲁棒性全部就绪
