...
> 文档角色：当前已验收实现事实的唯一真源
> 已验收版本：V1.4.2
> 当前开发版本：[V1.5.0 PLAN](./versions/v1.5.0/PLAN.md) 已于 2026-08-23 批准执行；开发须从执行时最新的公开 `origin/main` 建立基线
> V1.5.0 实现状态：尚未验收；本文下文仍只描述 V1.4.2 已验收事实
> 状态日期：2026-08-22
...
当前已发布产品基线由 annotated tag `v1.4.2` 标识，具体发布 commit 以 tag 解引用后的目标为唯一真值。公开 `main` 从该发布基线继续正常增量，允许包含不改变产品版本的发布后文档治理提交；这类提交不移动旧 tag，也不改变 V1.4.2 的源码验收结论。版本过程、失败候选、验收 commit 和发布修正分别保留在对应 RESULT；本文不重复版本流水账。
