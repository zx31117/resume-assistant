...
> 发布标识：最终发布后以 annotated tag `v1.5.0` 的目标 commit 为准；本文不回填最终文档提交自身 SHA
> 状态日期：2026-08-23
...
V1.5.0 已完成开发验证、验收 Agent 源码验收、人工确认和文档验收。源码验收绑定上方精确 commit；后续如修改相关源码、测试、配置或公开元数据，受影响范围必须重新验收。版本过程、失败候选、返工和测试证据保存在 [V1.5.0 RESULT](./versions/v1.5.0/RESULT.md)，本文不重复流水账。
